 ...
 *   SplitMate -> the app's display name
 *   모임·여행 정산을 초간단하게 — 더치페이 계산기에 리워드 광고로 '1/N 영수증' PDF 출력 잠금 해제    -> the one-line description
 */
...
export default function Home() {
  ...
  return (
    <ScreenScaffold
      top={<Top title={<Top.TitleParagraph>SplitMate</Top.TitleParagraph>} />}
    >
      {/* 시각 앵커: 헤드라인 + 카드 내 진입 버튼(부유 금지, display="block" 전체폭).
          데이터 앱이면 value를 <Amount typography="t1" />(핵심 숫자)로 교체하라. */}
      <SummaryHero
        label="SplitMate"
        value={<Paragraph.Text typography="t2">모임·여행 정산을 초간단하게 — 더치페이 계산기에 리워드 광고로 '1/N 영수증' PDF 출력 잠금 해제</Paragraph.Text>}
